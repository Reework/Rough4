...
public class FoodNapPDF extends AsyncTask<String, String, String> {
    private List<ClsCustomFoodNapData> transactionsDetails;
    ...
    public FoodNapPDF(ArrayList<ClsCustomFoodNapData> transactionsDetails, Context mContext, String month) {
...
//                    Toast.makeText(context, "PDF downloaded at : " + result, Toast.LENGTH_SHORT).show();
    ...
    private String generatePDF() {
        ...
        try {
            ...
            String pdfName = "/Food Discipline "+month  + ".pdf";
...
// Add item into Paragraph
            prHead.add("Food Descipline Details "+month +"\n\n\n");
...
// Create Table into Document with 1 Row
            float[] columnWidths = {2.4f, 2.4f,1.4f,1.4f,1.4f,1.4f,1.4f};
...
// Create New Cell into Table
            ...
            insertCell(table, "Before Breakfast(hh:mm) ", Element.ALIGN_CENTER, 1, bfBold14);
            insertCell(table, "Breakfast(hh:mm)", Element.ALIGN_CENTER, 1, bfBold14);
            insertCell(table, "Lunch(hh:mm)", Element.ALIGN_CENTER, 1, bfBold14);
            insertCell(table, "Evening Snacks(hh:mm)", Element.ALIGN_CENTER, 1, bfBold14);
            insertCell(table, "Dinner(hh:mm)", Element.ALIGN_CENTER, 1, bfBold14);
            insertCell(table, "Dessert(hh:mm)", Element.ALIGN_CENTER, 1, bfBold14);
...
//                String datess=  formatDates(transactionsDetails.get(i).getHealthdate().substring(0,index));
                ...
                insertCell(table, (String.valueOf(transactionsDetails.get(i).getBeforeBreakfast())).replace(".",":"), Element.ALIGN_CENTER, 1, bf_normal);
                insertCell(table, (String.valueOf(transactionsDetails.get(i).getBreakfast())).replace(".",":"), Element.ALIGN_CENTER, 1, bf_normal);
                insertCell(table, (String.valueOf(transactionsDetails.get(i).getLunch())).replace(".",":"), Element.ALIGN_CENTER, 1, bf_normal);
                insertCell(table, (String.valueOf(transactionsDetails.get(i).getEveningSnacks())).replace(".",":"), Element.ALIGN_CENTER, 1, bf_normal);
                insertCell(table, (String.valueOf(transactionsDetails.get(i).getDinner())).replace(".",":"), Element.ALIGN_CENTER, 1, bf_normal);
                insertCell(table, (String.valueOf(transactionsDetails.get(i).getDessert())).replace(".",":"), Element.ALIGN_CENTER, 1, bf_normal);
            }
