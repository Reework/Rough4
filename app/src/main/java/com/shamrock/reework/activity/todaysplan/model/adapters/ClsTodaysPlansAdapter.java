...
import com.shamrock.reework.activity.MyPlansModule.model.IdSorter;
...
public class ClsTodaysPlansAdapter extends RecyclerView.Adapter<ClsTodaysPlansAdapter.MyHolder> {
    ...
    public ClsTodaysPlansAdapter(Context context,
...
//                                onAddFoodPlan_listner.onNotificationClick(Integer.parseInt(dataList.get(adapterPosition).getId()),dataList.get(adapterPosition).isNotify(),isAdded, alternate);
    ...
    public MyHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(viewGroup.getContext()).
                inflate(R.layout.row_todays_plan, viewGroup, false);
                        ...
                        if (model.getGroups().get(j).getPlanItems()!=null){
                            try{
                                arylst_cuPlanItems.add(new PlanItems(model.getGroups().get(j).getPlanItems().get(k).getSubActivityId(),model.getGroups().get(j).getPlanItems().get(k).getActivationTime(),model.getGroups().get(j).getPlanItems().get(k).getMeasurement()
                                        ...
                                        , 0, model.isAdded(), Integer.parseInt(model.getId()), model.getGroups().get(j).getPlanItems().get(k).getLinkedRecipeId(), str_final_text.toString(),model.getGroups().get(j).getId(),Integer.parseInt(model.getDailyPlanMasterId()),model.getGroups().get(j).getPlanItems().get(k).isTodayPlan(),model.getGroups().get(j).getPlanItems().get(k).getReeworkerplanId(),model.getGroups().get(j).getPlanItems().get(k).getImagePath(),model.getGroups().get(j).getPlanItems().get(k).getVideolink()));
...
//                            for (int m = model.getGroups().get(j).getAlternateItems().size()-1; m >= 0; m--) {
            ...
            myHolder.recler_todays_plan_item.setAdapter(new ClsPlansDataAdapter(context, arylst_cuPlanItems, model.getDailyPlanMasterId(), model.getMealType()));
