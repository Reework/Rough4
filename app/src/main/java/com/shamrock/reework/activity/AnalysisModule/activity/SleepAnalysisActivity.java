...
public class SleepAnalysisActivity extends AppCompatActivity implements  DatePickerDialog.OnDateSetListener, AdapterView.OnItemSelectedListener, View.OnClickListener {
    Context context;
    Toolbar toolbar;
    Utils utils;
    ...
    ArrayList<SleepAnaysisData> tempList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sleep_analysis2);
        context= SleepAnalysisActivity.this;
        ...
        if (ISFromANnalysis){
            ...
            rd_button_sleep_quta.performClick();
            rd_button_sleep_nap.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    startActivity(new Intent(SleepAnalysisActivity.this, SleepNapMainActivity.class));
...
//                if (!tempList.isEmpty()){
//                    new GeneratePDF(tempList,SleepAnalysisActivity.this,spinner.getSelectedItem().toString()).execute();
//
//                }else {
//                    Toast.makeText(SleepAnalysisActivity.this, "No data available", Toast.LENGTH_SHORT).show();
//
//                }
            }
        });
...
//
//        }else {
//            tvTitle.setText("Trend Sleep Quota");
//
//        }
        utils = new Utils();
...
//        view.draw(canvas);
    ...
    private void showFullScreenDailog(final ArrayList<SleepAnaysisData> tempList1) {
...
//                    openFitBit();
                    ...
                    if (!tempList.isEmpty()){
                        ...
                        Dexter.withActivity(SleepAnalysisActivity.this)
...
//        request.setUserid(userId);
        ...
        Call<ClsSleepAnalysis> call = notificationService.getSleepActivityAnalysis(clsHistoryRequest);

        call.enqueue(new Callback<ClsSleepAnalysis>()
        {
            @Override
            public void onResponse(Call<ClsSleepAnalysis> call, Response<ClsSleepAnalysis> response)
            {
                utils.hideProgressbar();
                ...
                if (response.code() == Client.RESPONSE_CODE_OK)
                {
                    ClsSleepAnalysis listResponse = response.body();
                    ...
                    if (listResponse != null && listResponse.getCode().equals("1"))
                    {
                        ...
                        final ArrayList<SleepAnaysisData> tempList1 = new ArrayList<SleepAnaysisData>();
                        for(int i=0;i<tempList.size();i++){
                            if(!tempList.get(i).getActualSleep().equals("0.0")){
...
//                            finish();
            ...
            @Override
            public void onFailure(Call<ClsSleepAnalysis> call, Throwable t)
            {
                utils.hideProgressbar();
                Log.e("NotificationsActivity", t.toString());
            }
    ...
    private void setData(final ArrayList<SleepAnaysisData> aryCalories) {
...
//        ArrayList<Entry> yValues4=new ArrayList<>();
        ...
        for (int i = 0; i <aryCalories.size() ; i++) {
            yValues1.add(new Entry(i, Float.parseFloat(aryCalories.get(i).getActualSleep())));
            yValues2.add(new Entry(i,Float.parseFloat(aryCalories.get(i).getScheduledSleep())));
...
//            yValues4.add(new Entry(i,Float.parseFloat(aryCalories.get(i).getScheduledCalories())));
        ...
        set1=new LineDataSet(yValues1,"Actual sleep(Mins)");
        ...
        set2=new LineDataSet(yValues2,"Scheduled Sleep(Mins)");
...
//        set4.setLineWidth(2f);
        ...
        linegraph.getDescription().setText("Sleep");
